...
x_scale = 1.017
y_scale = 17
ref_dir = '/home/sean/Documents/thesis/ref_spectra/cellobiose'
...
for ax, dir in zip(plt.subplots(len(temp_dirs), sharex=True, sharey=True)[1][::-1], temp_dirs):
    ...
    freqs = [float(line[0]) for line in split_lines[1:] if float(line[0]) > 200]
    intens = [float(line[1]) for line in split_lines[1:] if float(line[0]) > 200]
