...
def suplabel(axis,label,label_prop=None,
    ...
    ymin = []
    for ax in fig.axes:
        xmin.append(plt.get_position().xmin)
        ymin.append(plt.get_position().ymin)
...
for dir in temp_dirs:
    ...
    freqs = [float(line[0]) for line in split_lines if float(line[0]) > 250]
    intens = [float(line[1]) for line in split_lines if float(line[0]) > 250]
    min_intens = min(intens)
    intens = [i - min_intens for i in intens]
    max_intens = max(intens)
    plt.plot(freqs, [i/max_intens for i in intens], color='r', label='Experimental')
    ...
    try:
        ...
        plt.plot([x*x_scale for x in x_array], [y/max_y for y in y_array], label='Simulated', color='k')
    except Exception as e:
        print(e)
    finally:
        os.chdir(top_dir)
    plt.text(0.1, 0.7, dir, horizontalalignment='center', verticalalignment='center', transform=plt.gca().transAxes)
